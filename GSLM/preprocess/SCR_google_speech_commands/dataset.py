...
class SpeechCommandsDataset(SpeechCommandsBaseDataset):
    ...
    def __init__(self, data_list, root_path, **kwargs):
        ...
        data = [
            (class_name, audio_path) if class_name in self.class2index.keys() else ("_unknown_", audio_path)
            for class_name, audio_path in data_list
        ]
        data += [
            ("_silence_", audio_path)
            for audio_path in Path(root_path, "speech_commands_v0.01", "_background_noise_").glob("*.wav")
        ]
...
def get_dataloader(dataset, batch_size, num_workers, drop_last=False, balanced=False):
    sampler = WeightedRandomSampler(dataset.sample_weights, len(dataset.sample_weights)) if balanced else None
