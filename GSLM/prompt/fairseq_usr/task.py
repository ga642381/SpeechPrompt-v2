...
@dataclass
class PromptLanguageModelingConfig(FairseqDataclass):
    data: Optional[str] = field(default=None, metadata={"help": "path to data directory"})
    ...
    dictionary: Dictionary = field(
        default=None, metadata={"help": "the dictionary for the input of the language model"}
    )
    targets: Optional[str] = field(
        default=None, metadata={"help": "list of the target types that the language model should predict.  Can be one of 'self', 'future', and 'past'."}
    )
    output_dictionary: Dictionary = field(
        default=None, metadata={"help": "the dictionary for the output of the language model"}
    )
    """
    output_dictionary_size: int = field(default=-1, metadata={"help": "limit the size of output dictionary"})
    ...
    # ========== prompt ========== #
    prompt_initialize: bool = field(
        default=False,
        metadata={"help": "whether initialize the weight of prompts with first batch of data"},
    )
...
@register_task("prompt_language_modeling", dataclass=PromptLanguageModelingConfig)
class PromptLanguageModelingTask(LegacyFairseqTask):
    """
    ...
    """
    def __init__(self, cfg: PromptLanguageModelingConfig) -> None:
        super().__init__(cfg)
        self.dictionary = self.cfg.dictionary
        self.output_dictionary = self.cfg.output_dictionary
        if self.cfg.targets == None:
            self.targets = ["future"]
        else:
    ...
    def train_step(self, sample, model, criterion, optimizer, update_num, ignore_grad=False):
        ...
        # ======================= #
        if self.args.prompt_initialize and not model.cfg.fine_tune and update_num == 0:
            print("initialize input embedding prompts")
            prompt_length = model.cfg.prompt_length
            input_embed = model(
                src_tokens=sample["net_input"]["src_tokens"], src_lengths=sample["net_input"]["src_lengths"]
            )[1]["embed"]
            input_embed = torch.mean(torch.mean(input_embed, dim=0), dim=0, keepdim=True)
            for w in model.decoder.prefix_embed_prompts.weight:
                w = input_embed

            if model.cfg.deep_prompt:
                print("initialize deep embedding prompts")
                attn = model(
                    src_tokens=sample["net_input"]["src_tokens"], src_lengths=sample["net_input"]["src_lengths"]
                )[1]["attn_layers"]
                for i in range(len(attn)):
                    input_attn = torch.mean(torch.mean(attn[i][prompt_length:], dim=0), dim=0, keepdim=True)
                    for k in model.decoder.deep_key_embed_prompts[i].weight:
                        k = input_attn
                    for v in model.decoder.deep_value_embed_prompts[i].weight:
                        v = input_attn
        else:
